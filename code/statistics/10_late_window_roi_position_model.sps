...
*
* Purpose:
*   Compute regional ROI mean amplitudes in the 250-500 ms window and test
*   whether the sequence-position effect varies across scalp regions.
*
* Input:
...
*
* Model:
*   Linear mixed-effects model with fixed effects of Domain, ROI, Position,
*   and their interactions. Position is modeled as an ordinal predictor
*   coded A = 0, B = 1, C = 2, D = 3.
*
*   A random slope for Position by participant is included.
* -------------------------------------------------------------------------.


* -------------------------------------------------------------------------.
* 1) Compute ROI mean amplitudes.
* -------------------------------------------------------------------------.
...
VARSTOCASES
  /MAKE Y FROM
    A_Acon_Fro A_Acon_Cen A_Acon_Par A_Acon_Occ
    A_Bcon_Fro A_Bcon_Cen A_Bcon_Par A_Bcon_Occ
    A_Ccon_Fro A_Ccon_Cen A_Ccon_Par A_Ccon_Occ
    A_Dcon_Fro A_Dcon_Cen A_Dcon_Par A_Dcon_Occ

    G_Acon_Fro G_Acon_Cen G_Acon_Par G_Acon_Occ
    G_Bcon_Fro G_Bcon_Cen G_Bcon_Par G_Bcon_Occ
    G_Ccon_Fro G_Ccon_Cen G_Ccon_Par G_Ccon_Occ
    G_Dcon_Fro G_Dcon_Cen G_Dcon_Par G_Dcon_Occ

    L_Acon_Fro L_Acon_Cen L_Acon_Par L_Acon_Occ
    L_Bcon_Fro L_Bcon_Cen L_Bcon_Par L_Bcon_Occ
    L_Ccon_Fro L_Ccon_Cen L_Ccon_Par L_Ccon_Occ
    L_Dcon_Fro L_Dcon_Cen L_Dcon_Par L_Dcon_Occ
  /INDEX = Cell(48)
  /KEEP = ERPset
  /NULL = KEEP.
...
* -------------------------------------------------------------------------.
* 3) Create Domain, Sequence Position, and ROI variables.
* -------------------------------------------------------------------------.
...
* Sequence position within each domain:
*   A = 1
*   B = 2
*   C = 3
*   D = 4.
...
* Ordinal sequence-position predictor:
*   A = 0
*   B = 1
*   C = 2
*   D = 3.
...
* -------------------------------------------------------------------------.
* 4) Linear mixed-effects model: ROI x Position.
* -------------------------------------------------------------------------.
...
* -------------------------------------------------------------------------.
* 5) Planned slope contrasts among ROIs.
*
* These contrasts compare the linear position slope between selected ROIs.
* ROI order is:
*   1 = Frontal
*   2 = Central
*   3 = Parietal
...
MIXED Y BY Domain ROI
  ...
  /RANDOM = Position | SUBJECT(ERPset) COVTYPE(VC)
  /PRINT = SOLUTION TESTCOV
  /LMATRIX 'Central vs Parietal slope'
